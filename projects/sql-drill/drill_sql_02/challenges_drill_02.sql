...
-- 24. Inteligência de Tempo: Extraia o ano e o mês da data de venda e mostre a receita total por mês (DATE_TRUNC ou EXTRACT).
-- 25. Crescimento Mensal (MoM): Calcule a variação percentual da receita em relação ao mês imediatamente anterior usando a função LAG().
-- 26. Agregação Condicional (Pivot): Mostre o gasto total de cada cliente, mas divida esse valor em colunas dedicadas por categoria (ex: Gasto_Meat, Gasto_Dairy, Gasto_Outros) usando SUM(CASE WHEN...).
...
-- 28. Tratamento de Nulos: Liste todos os clientes e os responsáveis pelo atendimento. Se não houver vendedor associado à venda, exiba a string 'Venda Direta Automática' (usando COALESCE ou IFNULL).
...
-- 30. Recência: Calcule a quantidade de dias entre a última compra de cada cliente e a data atual.
...
-- 33. Engajamento de Vendas: Identifique gaps de vendas calculando a quantidade de dias que cada vendedor ficou sem registrar nenhuma transação usando funções de janela sobre a data.
-- 34. Média Móvel (Rolling Average): Calcule a média móvel de faturamento dos últimos 3 dias para cada vendedor.
-- 35. Deduplicação de Registros: Usando CTE e ROW_NUMBER(), crie uma lógica que filtre transações duplicadas (mesmo cliente, produto e data), mantendo apenas o registro com o maior total faturado.
-- 36. Acumulado no Ano (YTD - Year-to-Date): Calcule o faturamento acumulado (Running Total) particionado por ano e por categoria, reiniciando a contagem a cada virada de ano.