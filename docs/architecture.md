...
Todos os volumes (`docker_postgres_data`, `minio_data`, `mongodb_data`, `redis_data`, `redisinsight_data`, `dremio_data`, `portainer_data`) são **externos** (`external: true` + `name:` fixo no compose), criados manualmente uma vez e nunca gerenciados pelo ciclo de vida do compose. Isso garante:
